...
var websocket = new WebSocket("ws://127.0.0.1:8080/GoBang_Online/game/" + gameInfo.userId);
...
function initGame() {
    ...
    function send(row, col) {
        ...
        //JS中可以在其他函数内部定义函数
        //java中可以在方法内定义其他内部类
        //JSON.parse 把json对象转成字符串
        // stringify 把字符串转回成json对象
        //java中使用的 Gson / Jackson
        //gson.toJson 把对象转成字符串
        //gson.fromJson 把字符串转成对象

        websocket.send(JSON.stringify(request));
    }
    // 新增处理服务器返回数据的请求
    // 并绘制棋子, 以及判定胜负
    //event参数是webSocket中自己已有的参数
    function handlerPutChess(event) {
        ...
        //response.userId服务器返回的userId,表示这个子是谁下的
        //gameInfo.userId
